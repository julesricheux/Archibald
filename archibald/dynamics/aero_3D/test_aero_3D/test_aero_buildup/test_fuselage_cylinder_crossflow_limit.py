...
def test_90_deg_crossflow():
    ...
    fuselage = make_fuselage(diameter=diameter, length=length, alpha_geometric=0)
    ...
    aero = asb.AeroBuildup(
        airplane=airplane, op_point=op_point, xyz_ref=np.array([length / 2, 0, 0])
    ).run()

    print(aero)

    from archibald.library.aerodynamics.viscous import Cd_cylinder

    CD_expected = Cd_cylinder(Re_D=op_point.reynolds(diameter), mach=op_point.mach())
    print(CD_expected)

    assert aero["CD"] == pytest.approx(CD_expected, rel=0.1)
