...
from archibald.toolbox import inspect_tools
...
class Opti(cas.Opti):
    ...
    def variable(
        ...
        Args:
            ...
            _stacklevel: Optional and advanced, purely used for debugging. Allows users to correctly track where
                variables are declared in the event that they are subclassing `archibald.Opti`. Modifies the
                stacklevel of the declaration tracked, which is then presented using
                `archibald.Opti.variable_declaration()`.
    ...
    def subject_to(
        ...
        Args:
            ...
            _stacklevel: Optional and advanced, purely used for debugging. Allows users to correctly track where
            constraints are declared in the event that they are subclassing `archibald.Opti`. Modifies the
            stacklevel of the declaration tracked, which is then presented using
            `archibald.Opti.constraint_declaration()`.
    ...
    def derivative_of(
        ...
        Args:
            ...
            constraints are declared in the event that they are subclassing `archibald.Opti`. Modifies the
            stacklevel of the declaration tracked, which is then presented using
            `archibald.Opti.variable_declaration()` and `archibald.Opti.constraint_declaration()`.
    ...
    def constrain_derivative(
        ...
        # TODO make
        from archibald.numpy.integrate_discrete import integrate_discrete_intervals
        ...
        for i in np.arange(len(g)):
            if constraint_violated[i]:
                ...
                if lbg_isfinite[i] and ubg_isfinite[i]:
                    ...
                    raise ValueError(
                        "Contact the archibald developers if you see this message; it should be impossible."
                    )
