...
class Wing(ArchibaldObject):
    ...
    def apply_operating_point(
        ...
        for i, xsec in enumerate(new_wing.xsecs):
            ...
            # Remove the component along yg
            c0_proj = c0 - np.dot(c0, yg) * yg
            c1_proj = c1 - np.dot(c1, yg) * yg
            
            c0_proj /= np.linalg.norm(c0_proj)
            c1_proj /= np.linalg.norm(c1_proj)
            
            # Signed rotation around yg
            angle = np.arctan2(
                np.dot(np.cross(c0_proj, c1_proj), yg),
                np.dot(c0_proj, c1_proj),
            )
            
            xsec.twist += np.degrees(angle)
        return new_wing
    ...
    def rotate_local(
        ...
        """
        Applies a local rotation to the Wing. The axis and origin are defined in the 
        boat's local reference frame (prior to global OperatingPoint transformations).
        
        Args:
            angle_deg: Rotation angle in degrees.
            axis: A 3-element sequence/array defining the directional vector of the axis of rotation.
            origin: A 3-element sequence/array defining the point the rotation axis passes through.
            
        Returns:
            A new Wing object with locally rotated cross-sections.
        """
        new_wing = self.deepcopy()
        ...
        for xsec in new_wing.xsecs:
            ...
            shifted = xsec.xyz_le - origin_arr
            xsec.xyz_le = (rot_mat @ shifted) + origin_arr
