...
class ArchibaldObject(ABC):
    _arch_metadata: dict[str, str] = None

    @abstractmethod
    def __init__(self):
        """
        Denotes ArchibaldObject as an abstract class, meaning you can't instantiate it directly - you must subclass
        (extend) it instead.
        """
        pass
    ...
    def __eq__(self, other):
        """
        Checks if two Archibald objects are value-equivalent. A more sensible default for classes that represent
    ...
    def save(
        ...
        Creates a .arch file, which is a binary file that can be loaded with `archibald.load()`. This can be loaded
            into memory in a different Python session or a different computer, and it will be exactly the same as when it
            was saved.

        Args:

            filename: The filename to save this object to. Should be a .arch file.

            verbose: If True, prints messages to console on successful save.

            automatically_add_extension: If True, automatically adds the .arch extension to the filename if it doesn't
        ...
        if filename.suffix == "" and automatically_add_extension:
            filename = filename.with_suffix(".arch")

        if verbose:
            print(f"Saving {str(self)} to:\n\t{filename}...")

        import archibald as arch
...
def load(
    ...
    """
    Loads an ArchibaldObject from a file.

    Upon load, will compare metadata from the file to the current Python version and Archibald version. If there are
    any discrepancies, will raise a warning.

    Args:

        filename: The filename to load from. Should be a .arch file.

        verbose: If True, prints messages to console on successful load.

    Returns: An ArchibaldObject.
    ...
    # Check if the archibald version is different
    import archibald as arch

    try:
        saved_arch_version = metadata["arch_version"]

        if saved_arch_version != arch.__version__:
            warnings.warn(
                f"This object was saved with aArchibald {saved_arch_version}, but you are currently using Archibald {arch.__version__}.\n"
                f"This may cause compatibility issues.",
                stacklevel=2,
            )
        ...
        warnings.warn(
            "This object was saved without Archibald version info metadata. This may cause compatibility issues.",
            stacklevel=2,
        )
...
class ExplicitAnalysis(ArchibaldObject):
    default_analysis_specific_options: dict[type, dict[str, Any]] = {}
    """This is part of archibald's "analysis-specific options" feature, which lets you "tag" geometry objects with 
    ...
    def get_options(
        self,
        geometry_object: ArchibaldObject,
    ) -> dict[str, Any]:
        """
        ...
        Args:
            geometry_object: An instance of an Archibald geometry object, such as an Airplane, Wing, etc.

                * In order for this function to do something useful, you probably want this option to have
                `analysis_specific_options` defined. See the arch.Airplane constructor for an example of this.
...
class ImplicitAnalysis(ArchibaldObject):
    @staticmethod
    def initialize(init_method):
        ...
        an optimization environment (arch.Opti type) that it can work in.

        How do we obtain an arch.Opti environment to work in? Well, this decorator adds an optional `opti` parameter to
        the __init__ method that it is applied to.

            1. If this `opti` parameter is not provided, then a new empty `arch.Opti` environment is created and stored as
            `ImplicitAnalysis.opti`.

            2. If the `opti` parameter is provided, then we simply assign the given `arch.Opti` environment (which may
