...
import os
import subprocess
import archibald1.numpy as np

from archibald2.tools.math_utils import *
from archibald2.geometry.legacy.lifting_planes import Spi, Rudder
...
def write_avl_input(objectSet, aoa=0.):
    ...
    f.write("LOAD {0}\n".format('.'+objectSet._AVLgeo[len(objectSet._abs_dir):]))
    ...
    f.write("FS {0}\n".format('.'+objectSet._AVLout[len(objectSet._abs_dir):]))
    f.write("O\n")
...
def run_avl_analysis(objectSet):
    ...
    if os.path.exists(objectSet._AVLin):
        ...
        subprocess.run("avl.exe < "+objectSet._AVLin, shell=True)
