# -*- coding: utf-8 -*-
"""
Created on Tue Jan 14 15:08:35 2025
...
addStl = 'C:/Users/jrich/Documents/archibald-main/Private/n169pctc_data/add.stl' # path to a STL mesh of the hull
...
aoa0 = 20. # deg # to be varied depending on awa 
...
leeway = 15. # deg # positive for windward / negative for leeward
# heel = -4. * setSail # deg # positive for windward / negative for leeward
heel = - tws0i/12. * setSail # deg # positive for windward / negative for leeward
heel = 0.
...
rudderAngle = 0. # positive to luff (push the helm) / negative to bear away (pull the helm)
finAngle = 15. * setSail
...
aoa = np.array([
    1.+ 2*boom_correction_from_awa(awa0, a=a, b=b, fac=fac), # boom incidence correction
    1.+ boom_correction_from_awa(awa0, a=a, b=b, fac=fac), # boom incidence correction
    1.]) * aoa_corr # deg
...
hull = Hull('n169pctc_hull', displacement, cog, hullStl)
...
rudderDxf = 'C:/Users/jrich/Documents/archibald-main/Private/n169pctc_data/rudder.dxf'
...
for side in ['p', 's']:
    for row in ['a', 'f']:
        DXFS.append('C:/Users/jrich/Documents/archibald-main/Private/n169pctc_data/fin_'+side+'_'+row+'.dxf')

rudderLe, rudderChords = dxf_to_le_chords(rudderDxf, nSections, method='bezier')
rudderLe[:,0] *= -1.

for finDxf in DXFS:
    finLe, finChords = dxf_to_le_chords(finDxf, nSections, method='bezier')
    finLe[:,0] += finDx
    finLe[:,0] *= -1.
    
    LES.append(finLe)
    CHORDS.append(finChords)
...
n169app = Appendage(
    ...
    wings=[
        ...
        Fin(
            ...
            xyz_le = LES[i],  # Coordinates of the XSec's leading edge, relative to the wing's leading edge.
            chords = CHORDS[i],
            airfoils = finAirfoilVec  # Airfoils are blended between a given XSec and the next one
        )
    for i in range(len(DXFS))] * finsOut
)
...
n169pctc.transform(heel=heel,
               trim=trim,
               leeway=leeway,
               dz=dz)

n169pctc.app.wings[0].local_rotation(deflection=rudderOffset + rudderAngle)

for i in range(len(DXFS)*finsOut):
    n169pctc.app.wings[i+1].local_rotation(deflection=-finAngle)


for i in range(nSolidsail):
    # MAINSAIL SETTING
    main_i = n169pctc.rig.wings[i]
    ...
    # JIBSAIL SETTING
    jib_i = n169pctc.rig.wings[nSolidsail+i]
...
if engineOn:
    ...
    Fprop, Mprop, prop = n169pctc.compute_propeller(
        rpm,
        op_point, 
        recompute_statics=False,
        full_output=True,
...
# AR = n169rig.wings[0].aspect_ratio()
# span = n169rig.wings[0].span()
...
Fsail, Msail, aero = n169pctc.compute_sails(
    op_point,
    nSpanwise=1,
    nChordwise=5,
    full_output=True,
...
# else:
#     coe = n169rig.aerodynamic_center()
#     zFoot = zSpardeck + mainLe[0,2]
#     zHead = zSpardeck + mainLe[-1,2]
#     zMid = zFoot + zHead / 2.
    
#     awa = op_point.awa(zMid) # in deg
#     aws = op_point._aws(zMid) # in m/s
    
#     CL = 0.0
#     CD = 0.05
    
#     CX = CL * np.sind(awa0) - CD * np.cosd(awa0)
#     CY = CL * np.cosd(awa0) + CD * np.sind(awa0)
    
#     dyn_pres = 0.5 * op_point.environment.air.density * n169rig.s_ref * aws**2
    
#     Fx = dyn_pres * CX
#     Fy = dyn_pres * CY
    
#     Fsail = np.array([Fx, Fy, 0.]) * sailingVessel
#     Msail = np.cross(coe, Fsail)


# %% APPENDAGE COMPUTATION

# AR = n169app.wings[0].aspect_ratio()
# span = n169app.wings[0].span()

cellAR = 2

if finsOut:
    Fapp, Mapp, hydro = n169pctc.compute_appendages(
        op_point,
        nSpanwise=1,
        nChordwise=3,
        full_output=True
...
Fw, Mw = n169pctc.compute_weight(op_point)
...
if viz:
    ...
    # print()
    # print(Fsail)
    # print(n169pctc.aeroVLMdata.lift_force_underway)
    # print(Fsail/n169pctc.aeroVLMdata.lift_force_underway)
